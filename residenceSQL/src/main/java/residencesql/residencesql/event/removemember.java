    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try(Connection conn = DriverManager.getConnection(DB_URL, DB_USERNAME, DB_PASSWORD);
            ...
            String sqldata = "UPDATE residence " +
                    "SET members = REPLACE (members, '"+args[0]+"', '') WHERE residencename in ('"+args[1]+"')";
            String sqldata2 = "UPDATE residence " +
                    "SET membersUUID = REPLACE(membersUUID, '" + id + "', '') WHERE residencename in ('"+args[1]+"')";
            stmt4.executeUpdate(sqldata);
            stmt5.executeUpdate(sqldata2);
            p.sendMessage(ChatColor.GREEN + "成功移除該成員");
